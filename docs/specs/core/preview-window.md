...
- [ ] Installing peep.nvim shall default to fetching a prebuilt companion binary for Linux/macOS/Windows, requiring no separate build step in the common case; building the companion binary locally shall remain available as a documented fallback, not disallowed, for platforms/architectures with no matching prebuilt asset.
- [ ] The plugin may require the user to separately install a small number of OS-specific runtime dependencies (e.g. a system webview library on Linux) that cannot be bundled; any such dependency shall be clearly documented per-OS in install instructions, and its absence at runtime shall be detected and reported with a specific, descriptive error rather than hanging, crashing, or failing silently.
