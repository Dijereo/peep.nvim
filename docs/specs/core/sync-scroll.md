...
source buffer: markdown, Quarto (qmd), LaTeX (tex), and Jupyter
notebooks (ipynb). Does not apply to formats with no separate source
buffer (e.g. standalone PDF, images).
...
- [ ] Applies to source-backed formats: markdown, Quarto (qmd), LaTeX (tex), Jupyter notebooks (ipynb) — not standalone formats with no source buffer (e.g. PDF, images)
